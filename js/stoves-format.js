...
(function(){
  ...
  const diagram='assets/stoves-data/stove-plan-01.png';
  ...
  function apply(){
    tasks.filter(t=>t.practicalType==='stoves').forEach(t=>{
      ...
      if(t.number===5&&!String(t.text||'').includes('stove-task-diagram')){
        t.text += `<div class="stove-task-diagram" style="margin:16px 0;text-align:center"><img src="${diagram}" alt="Схема кожуха печи" style="display:block;max-width:420px;width:100%;height:auto;margin:0 auto" loading="lazy"></div>`;
      }
    });
  }
  apply();
