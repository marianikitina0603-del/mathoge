...
(function(){
  ...
  function normalizePrintedDiagrams(d){
    d.querySelectorAll('#examPaper .number11-diagram').forEach(img=>{
      important(img,'display','block');important(img,'float','none');important(img,'width','auto');important(img,'height','auto');important(img,'max-width','68mm');important(img,'max-height','35mm');important(img,'object-fit','contain');important(img,'object-position','center');important(img,'margin','2mm auto');
    });
    d.querySelectorAll('#examPaper .number15-task-layout,#examPaper .number16-task-layout,#examPaper .number17-task-layout,#examPaper .number18-task-layout').forEach(layout=>{
      ...
      important(layout,'display','flow-root');important(layout,'min-width','0');important(layout,'width','100%');
    });
    d.querySelectorAll('#examPaper .number15-diagram,#examPaper .number16-diagram,#examPaper .number17-diagram,#examPaper .number18-diagram').forEach(img=>{
      img.removeAttribute('width');img.removeAttribute('height');important(img,'float','right');important(img,'display','block');important(img,'width','38mm');important(img,'height','28mm');important(img,'min-width','0');important(img,'min-height','0');important(img,'max-width','38mm');important(img,'max-height','28mm');important(img,'object-fit','contain');important(img,'object-position','center');important(img,'margin','0 0 2mm 4mm');important(img,'break-inside','avoid');important(img,'page-break-inside','avoid');
    });
  }
  ...
  function rebuildFullPageSolutionGrids(d){
    ...
    for(let number=20;number<=25;number++){
      const task=d.querySelector(`#examPaper .preview-task[data-task-number="${number}"]`);if(!task)continue;
      const svg=task.querySelector('.solution-grid-svg');if(!svg)continue;
      const w=180,h=210,step=5;svg.setAttribute('viewBox',`0 0 ${w} ${h}`);svg.setAttribute('preserveAspectRatio','none');
      let markup='';
      for(let x=0;x<=w;x+=step)markup+=`<line x1="${x}" y1="0" x2="${x}" y2="${h}" stroke="#c7c7c7" stroke-width="0.32" vector-effect="non-scaling-stroke"/>`;
      for(let y=0;y<=h;y+=step)markup+=`<line x1="0" y1="${y}" x2="${w}" y2="${y}" stroke="#c7c7c7" stroke-width="0.32" vector-effect="non-scaling-stroke"/>`;
      markup+=`<rect x="2" y="2" width="23" height="7" fill="#fff"/><text x="3" y="7" font-size="4.2" font-family="Arial, sans-serif" font-weight="700" fill="#000">Решение:</text>`;
      svg.innerHTML=markup;
    }
  }

  async function printWithSolutions(){
    const source=document.getElementById('examPaper');if(!source)return;
    ...
    try{
      ...
      const overrides=`
        ...
        #examPaper{display:block!important;width:auto!important;max-width:none!important;min-height:0!important;margin:0!important;padding:0!important;border:0!important;box-shadow:none!important;background:#fff!important;color:${PRINT_TEXT_COLOR}!important}
        #examPaper,#examPaper *{color:${PRINT_TEXT_COLOR}!important;-webkit-text-fill-color:${PRINT_TEXT_COLOR}!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
        #examPaper svg:not(.solution-grid-svg) text{fill:${PRINT_TEXT_COLOR}!important}
        #examPaper mjx-container,#examPaper mjx-container *{color:${PRINT_TEXT_COLOR}!important;-webkit-text-fill-color:${PRINT_TEXT_COLOR}!important}
        #examPaper mjx-container[jax="SVG"] svg,#examPaper mjx-container[jax="SVG"] svg *{color:${PRINT_TEXT_COLOR}!important}
        #examPaper mjx-container[jax="SVG"] svg path,#examPaper mjx-container[jax="SVG"] svg use{fill:${PRINT_TEXT_COLOR}!important;stroke:none!important}
        ...
        #examPaper mjx-container{visibility:visible!important;opacity:1!important;color:${PRINT_TEXT_COLOR}!important}
        #examPaper mjx-container[jax="SVG"]{display:inline-block!important;max-width:100%!important;color:${PRINT_TEXT_COLOR}!important}
        #examPaper mjx-container[display="true"]{display:block!important}
      `;
      d.open();d.write(`<!doctype html><html lang="ru"><head><meta charset="utf-8">${links}${styles}${mathStyles}<style>${overrides}</style></head><body><div style="display:none" aria-hidden="true">${mathCache}</div>${clone.outerHTML}</body></html>`);d.close();
      normalizePrintedDiagrams(d);rebuildFullPageSolutionGrids(d);await waitForImages(d);normalizePrintedDiagrams(d);rebuildFullPageSolutionGrids(d);
      if(d.fonts&&d.fonts.ready){try{await d.fonts.ready;}catch(e){}}
      await new Promise(resolve=>(iframe.contentWindow.requestAnimationFrame||requestAnimationFrame)(()=>resolve()));normalizePrintedDiagrams(d);rebuildFullPageSolutionGrids(d);
      iframe.contentWindow.focus();iframe.contentWindow.print();setTimeout(()=>iframe.remove(),1500);
    }catch(err){iframe.remove();console.error('Не удалось подготовить печать с решением',err);if(typeof toast==='function')toast('Не удалось подготовить печать. Повторите ещё раз.');}
  }

  document.addEventListener('click',event=>{const btn=event.target.closest('#printVariant');if(!btn)return;event.preventDefault();event.stopImmediatePropagation();printWithSolutions();},true);
})();