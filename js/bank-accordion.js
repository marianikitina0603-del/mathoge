...
function formatPracticalText(t){
  if(t.number===1)return formatTask1Table(t.text);
  if(t.number===5)return formatTask5Table(t.text);
  return t.text;
}
...
function togglePracticalSet(typeKey,setNo){
  ...
  if(allAdded){const ids=new Set(setTasks.map(t=>t.id));variant=variant.filter(t=>!ids.has(t.id));toast(`Комплект ${setNo} удалён из варианта`);}
  else{const existing=new Set(variant.map(t=>t.id));setTasks.forEach(t=>{if(!existing.has(t.id))variant.push(t)});toast(`Комплект ${setNo} добавлен в вариант`);}
  saveCurrent();renderBank();
}
...
function renderPracticalStructure(){
  return `<details class="number-accordion practical-number-accordion"><summary class="number-summary"><span class="accordion-chevron">›</span><span class="number-badge">№1–5</span><span class="number-title">Практические задания</span><span class="number-stats">7 типов</span></summary><div class="number-content practical-types">${practicalSetStructure.map(type=>{
    ...
    return `<details class="prototype-accordion practical-type-accordion"><summary class="prototype-summary"><span class="accordion-chevron">›</span><span class="prototype-main"><strong>${type.title}</strong></span><span class="analog-count">${setNumbers.length} комплектов</span></summary><div class="prototype-content practical-sets">${setNumbers.length?setNumbers.map(setNo=>{
      const setTasks=typeTasks.filter(t=>t.set===setNo).sort((a,b)=>a.number-b.number);const context=setTasks.find(t=>t.context)?.context||'';const setAdded=isPracticalSetAdded(setTasks);
      return `<details class="analogs-accordion practical-set-accordion"><summary class="analogs-summary practical-set-summary"><span class="accordion-chevron">›</span><strong>Комплект ${setNo}</strong><span>${setTasks.length} задач</span><button type="button" class="add-button set-add-button ${setAdded?'added':''}" data-add-set="${type.key}:${setNo}">${setAdded?'✓ Добавлено':'+ В вариант'}</button></summary><div class="practical-set-body">${context?`<div class="practical-context"><div class="practical-context-title">Общее условие к заданиям 1–5</div>${context}<div class="plan-note">План относится к общему условию и будет перенесён из исходного PDF отдельным изображением.</div></div>`:''}<div class="analogs-list practical-set-list">${setTasks.map(t=>taskCard(t)).join('')}</div></div></details>`;
    }).join(''):'<div class="empty-bank compact-empty">Комплекты будут добавлены из банка ФИПИ.</div>'}</div></details>`;
  }).join('')}</div></details>`;
}

function renderStandardNumber(number){
  const numberTasks=tasks.filter(t=>t.number===number);const prototypes=numberTasks.filter(t=>t.kind==='Прототип').sort((a,b)=>a.prototype-b.prototype);const topic=numberTasks[0]?.topic||'';
...
function renderBank(){
  const root=$('#taskList');const standardNumbers=[...new Set(tasks.filter(t=>t.number>=6).map(t=>t.number))].sort((a,b)=>a-b);$('#resultCount').textContent=tasks.length;root.innerHTML=renderPracticalStructure()+standardNumbers.map(renderStandardNumber).join('');
  $$('#taskList [data-add]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();toggleTask(b.dataset.add)});
  $$('#taskList [data-add-set]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();const [typeKey,setNo]=b.dataset.addSet.split(':');togglePracticalSet(typeKey,Number(setNo));});
}
